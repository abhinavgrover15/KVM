...
require Exporter;
@ISA = qw(Exporter);
@EXPORT_OK = qw(&write_log $PATH_TO_APPLIANCE_LOG &fail &success &initialize &search &addKVMACHINE &delKVMACHINE &addVIRTUALMACHINE &assignPIP &rebootVM &checkUPTIME &enableprivateNET &createnetworkXML &disprivNETWORK &vmCLONE &createBACKUP &createcustomIMAGE &delBACKUP &delcustomIMAGE &vmREVERTSS &vmSNAPSHOT);
#
...
if (!-f "/etc/libvirt/qemu/$cust_id-$server_id.xml")
{
    ...
    system("virsh list | grep -i $server_id");  ###### Check for guest machine state
       if ($? == 0)
         {
...
else { write_log(" Diiferent Type of OS than Linux"); }
}
}
...
{
system("qemu-img create -f qcow2  -o preallocation=metadata /var/lib/libvirt/images/$cust_id-$server_id.qcow2 20$dm  ");
system("virt-install --connect qemu:///system --name $cust_id-$server_id --ram $ram --vcpus $vcpu --disk /var/lib/libvirt/images/$cust_id-$server_id.qcow2,format=qcow2 --graphics vnc,password=$vnc_password,port=$vm_host_vnc_port,listen=$host_ip_address --cdrom $iso_path --network network:phybrid,mac=$wan_mac_address ");
system("virsh destroy $cust_id-$server_id ");
if ($? != 0 ) {
...
#system("cp /home/images/$os_name-$os_version-$os_arch.qcow2 /var/lib/libvirt/images/$cust_id-$server_id.qcow2 >/dev/null");
system("cp /mnt/$os_image_location/$os_name-$os_version-$os_arch.qcow2 /var/lib/libvirt/images/$cust_id-$server_id.qcow2");
system("sleep 10");
############ Check for HDD Size to be increased ###########################################
if ( $storage != 20 ) {
system("cp -rp /var/lib/libvirt/images/$cust_id-$server_id.qcow2 /home/backupimages/$cust_id-$server_id.qcow2 ");
system("qemu-img create -f qcow2 -o preallocation=metadata  /home/tempimages/$cust_id-$server_id.qcow2 $storage$dm ");
system("virt-resize --expand /dev/sda1 /var/lib/libvirt/images/$cust_id-$server_id.qcow2 /home/tempimages/$cust_id-$server_id.qcow2  >/dev/null");
system("mv /home/tempimages/$cust_id-$server_id.qcow2 /var/lib/libvirt/images/$cust_id-$server_id.qcow2");
...
system("cp /mnt/$cust_id-$server_id/etc/shadow /mnt/$cust_id-$server_id/etc/shadow-bak ");   ###### Backup Shadow File
...
system("mv /etc/libvirt/qemu/$cust_id-$server_id.xml /etc/libvirt/qemu/$cust_id-$server_id.xml-bak ");
system("virsh undefine $cust_id-$server_id");
system("mv /etc/libvirt/qemu/$cust_id-$server_id.xml-bak /etc/libvirt/qemu/$cust_id-$server_id.xml ");
system("virsh define /etc/libvirt/qemu/$cust_id-$server_id.xml ");
#####################################################################################################################
...
my $kvm_host_ips=shift;
           ...
           system("virsh attach-interface $cust_id-$server_id --type network --source $cust_id --persistent --mac '$lan_mac_address' ");
...
my $lan_mac_address=shift;
	...
	if (-e "/etc/libvirt/qemu/$cust_id-$server_id.xml")
	   ...
	   system("virsh detach-interface --domain $cust_id-$server_id --type network --mac $lan_mac_address --persistent");
