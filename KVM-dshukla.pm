...
system("cp -rp /var/lib/libvirt/images/$cust_id-$server_id.qcow2 /home/backupimages/$cust_id-$server_id.qcow2 > /dev/null");
system("qemu-img create -f qcow2 -o preallocation=metadata  /home/tempimages/$cust_id-$server_id.qcow2 $storage$dm > /dev/null");
	if ($os_type eq "linux")
	{ 
	...
	{
	 system("virt-resize --expand /dev/sda2 /var/lib/libvirt/images/$cust_id-$server_id.qcow2 /home/tempimages/$cust_id-$server_id.qcow2 >/dev/null");
	}

system("mv /home/tempimages/$cust_id-$server_id.qcow2 /var/lib/libvirt/images/$cust_id-$server_id.qcow2 > /dev/null");
...
my $file="/mnt/$cust_id-$server_id/etc/network/interfaces";
	$line6="auto eth0";
        ...
        $line5="dns-nameservers 204.80.91.59";
...
my $ram=$ram*1024;
my $dm="G";
my $host_ip_address="162.222.32.101";
 ...
 if ($os_type eq "linux") {
  ...
  system("rm -rf /mnt/$cust_id-$server_id/etc/sysconfig/network-scripts/ifcfg-eth0:*");
   ...
   for ($i=0; $i<$ipsremno; $i++) {
     ...
     write_log(" Some other Operating Sytem Found");
       return fail;
           }
}
